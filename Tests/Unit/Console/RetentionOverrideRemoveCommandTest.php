...
/**
 * @covers \Vortos\Scheduler\Console\RetentionOverrideRemoveCommand
 */
final class RetentionOverrideRemoveCommandTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->retentionStore->save(new RunRetentionOverride('tenant-1', 90, 'admin-1', new DateTimeImmutable()));
    }
    ...
    public function test_remove_succeeds_and_exits_zero(): void
    {
        $tester = new CommandTester(new RetentionOverrideRemoveCommand($this->makeService()));

        $tester->execute(['--tenant' => 'tenant-1', '--actor' => 'admin-1']);

        self::assertSame(0, $tester->getStatusCode());
        self::assertNull($this->retentionStore->find('tenant-1'));
    }

    public function test_remove_access_denied_exits_failure(): void
    {
        $this->policy->deny();
        $tester = new CommandTester(new RetentionOverrideRemoveCommand($this->makeService()));

        $tester->execute(['--tenant' => 'tenant-1', '--actor' => 'user-1']);

        self::assertSame(1, $tester->getStatusCode());
        self::assertStringContainsString('Access denied', $tester->getDisplay());
        self::assertNotNull($this->retentionStore->find('tenant-1'));
    }
}
