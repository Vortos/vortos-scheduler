...
/**
 * @covers \Vortos\Scheduler\Console\SchedulePauseCommand
 */
final class SchedulePauseCommandTest extends TestCase
{
    ...
    private function seedSchedule(?string $tenantId = null): Schedule
    {
        $s = new Schedule(
    ...
    public function test_pause_succeeds_and_exits_zero(): void
    {
        $schedule = $this->seedSchedule();
        $command  = new SchedulePauseCommand($this->makeService());
        ...
        self::assertStringContainsString('paused successfully', $tester->getDisplay());
    }

    public function test_pause_outputs_schedule_name(): void
    {
        $schedule = $this->seedSchedule();
        $command  = new SchedulePauseCommand($this->makeService());
        $tester   = new CommandTester($command);
    ...
    public function test_pause_with_unknown_id_exits_failure(): void
    {
        $command = new SchedulePauseCommand($this->makeService());
        $tester  = new CommandTester($command);
    ...
    public function test_pause_access_denied_exits_failure(): void
    {
        $schedule = $this->seedSchedule();
        $this->policy->deny();
