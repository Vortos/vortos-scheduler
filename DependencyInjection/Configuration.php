    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $treeBuilder->getRootNode()
            ...
            ->children()
                ...
                ->end()
                ->integerNode('fire_lease_sec')
                    ->defaultValue(900)
                    ->info('FB-64: a fire still processing this long after its claim is presumed abandoned by a dead consumer and reclaimed. Must exceed the longest scheduled command.')
                ->end()
                ->integerNode('fire_stranded_max_age_sec')
                    ->defaultValue(86400)
                    ->info('FB-64: an abandoned fire older than this is marked failed instead of re-run')
                ->end()
                ->integerNode('consume_memory_limit_mib')
                    ->defaultValue(0)
                    ->info('scheduler:consume --loop recycles once real memory reaches this many MiB. 0 = 80% of memory_limit, -1 = never')
                ->end()
                ->integerNode('consume_time_limit_sec')
                    ->defaultValue(3600)
                    ->info('scheduler:consume --loop recycles after this many seconds. 0 = never')
                ->end()
